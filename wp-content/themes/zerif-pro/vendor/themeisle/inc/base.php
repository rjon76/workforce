...
if ( ! class_exists( "THEMEISLE_SDK" ) ) :
	class THEMEISLE_SDK {
		...
		/**
		 * @var string $store_url Should contain the product type, either theme or plugin
		 */
		public $product_type;
		...
		public function __construct( $data = array() ) {
			...
			if ( ! $this->wordpress_available ) {
				...
				$logger->start();
				$licenser = new THEMEISLE_LICENSE( $this->product_name, $this->product_slug, $this->product_version, $this->product_type, $this->paid, $this->store_url, $this->product_data );
				$licenser->enable();
			}
